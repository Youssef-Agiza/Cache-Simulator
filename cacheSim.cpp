...
public:
    ...
    void Initialize(unsigned int lineSize, unsigned int numberOfLines, const unsigned int numberOfTagBits)
    {
        numberOfWords = (lineSize / 4 == 0) ? 1 : lineSize / 4;
        setTags = new unsigned int[numberOfLines];      // in words
        memset(setTags, 0 ,numberOfLines);
        validBits = new unsigned short[numberOfLines];      // in words
        memset(validBits, 0 ,numberOfLines);                  //All are invalid
        setIndex = 0;
        this->numberOfLines = numberOfLines;
    }

    bool IsInSet(unsigned int tag, int* index = nullptr)
    {
        for(uint32_t i = 0; i < numberOfLines; i++)
        {
            if(validBits[i] == VALID){
                if(setTags[i] == tag)
                {
                    if(index != nullptr)
                        *index = i;
    ...
    {
        unsigned int tag = address >> (32 - numberOfTagBits);
        setTags[setIndex] = tag;
        validBits[setIndex] = VALID;
        setIndex = (setIndex + 1) % numberOfLines;
    }
    ~Set()
    {
        delete[] setTags; 
...
private:
    unsigned int numberOfTagBits;
    unsigned int numberOfByteOffsetBits;
    unsigned int* setTags;
    unsigned short* validBits;
    unsigned int setIndex;
    unsigned int numberOfLines;
    unsigned int numberOfWords;
    enum Validity { NOT_VALID = 0, VALID = 1};
};

class N_WayCache{
public:
    N_WayCache(unsigned int numberOfWays, unsigned int blockSize)  
...
private:
    void Initialize()
    {
        unsigned int numberOfLines = CACHE_SIZE / m_BlockSize;                      // gives out the number of lines
        unsigned int numberOfTagBits = 32 - (unsigned int)(log2(m_BlockSize));      // 32 bit address - the number of bits we have for the words
        m_Sets = new Set[m_NumberOfWays];
        for(int i = 0; i < m_NumberOfWays; i++)
            m_Sets[i].Initialize(m_BlockSize, numberOfLines, numberOfTagBits);
    }
