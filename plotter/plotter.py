...
def plot(col_list,filename="plotter/EXPERIMENT.csv"):
    ...
    x_values=np.array(( dp[col_list[0]])).astype(str).tolist() #get values for x-axis from the first column

    for i in range(1,len(col_list)):
      value=np.array(dp[col_list[i]]) #converts data to array 
      plt.plot(x_values,value,marker="o",label="MemGen"+col_list[i]) #and use it in the plot
...
def main():
    ...
    if(len(sys.argv)>=4): x_col_title=sys.argv[3]
    elif(int(sys.argv[1])==1): x_col_title="line sizes"
    else: x_col_title="ways"
